...
configure_fail2ban() {
  log "Configuring fail2ban sshd and TAKlite auth jails"
...
port = ${TAKLITE_HTTP_HOST_PORT},${TAKLITE_HTTPS_HOST_PORT}
maxretry = 8
findtime = 10m
bantime = 1h
EOF
...
if [[ ! "\${RELEASE_ZIP_URL}" =~ ^https://github\\.com/C-OneThirty7/TAKlite/releases/download/v[0-9]+\\.[0-9]+\\.[0-9]+/[A-Za-z0-9_.-]+\\.zip$ ]]; then write_status "failed" "Invalid verified release zip URL"; rm -f "\${PROCESSING_FILE}"; exit 1; fi
...
ATAK HTTPS truststore:
  packaged inside each per-user connection .dp.zip
