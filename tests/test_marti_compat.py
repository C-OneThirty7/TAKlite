...
class MartiCompatibilityTests(unittest.TestCase):
    ...
    def test_missionupload_accepts_atak_plain_name_and_stores_zip_name(self):
        ...
        self.assertIn(b"/Marti/api/sync/metadata/", response)
        self.assertIn(b"/tool", response)
        packages = self.service.list_packages()
        self.assertEqual(len(packages), 1)
        self.assertEqual(packages[0]["Name"], "maps.zip")

    def test_missionquery_returns_metadata_tool_url_for_existing_package(self):
        payload = datapackage_bytes("mission-query")
        ...
        self.service.upsert_package("queryhash", "query.dp.zip", "ANDROID-1", payload, "http://127.0.0.1")

        status, _, response = self.request("GET", "/Marti/sync/missionquery?hash=queryhash")

        self.assertEqual(status, 200, response.decode("utf-8", "replace"))
        self.assertEqual(
            response.decode("utf-8"),
            "https://10.66.66.1:18443/Marti/api/sync/metadata/queryhash/tool",
        )

    def test_sync_search_returns_atak_query_result_shape(self):
        payload = datapackage_bytes("search")
        self.service.upsert_package("searchhash", "search.dp.zip", "ANDROID-1", payload, "http://127.0.0.1")
        ...
        self.assertEqual(item["Name"], "search.dp.zip")
        self.assertEqual(item["Hash"], "searchhash")
        self.assertIsInstance(item["PrimaryKey"], int)
        self.assertIsInstance(item["SubmissionDateTime"], str)
        self.assertIsInstance(item["SubmissionUser"], str)
        self.assertIsInstance(item["CreatorUid"], str)
        self.assertIsInstance(item["Keywords"], str)
        self.assertIsInstance(item["MIMEType"], str)
    ...
    def test_sync_metadata_tool_get_downloads_datapackage(self):
        ...
        status, headers, response = self.request("GET", "/Marti/api/sync/metadata/downloadhash/tool")

        self.assertEqual(status, 200, response.decode("utf-8", "replace"))
        self.assertEqual(response, payload)
        self.assertEqual(headers["Content-Type"], "application/x-zip-compressed")
        self.assertIn('filename="download-map.zip"', headers["Content-Disposition"])

    def test_sync_metadata_tool_head_reports_datapackage_headers(self):
        payload = datapackage_bytes("metadata-head")
        self.service.upsert_package("headhash", "head-map.zip", "ANDROID-1", payload, "http://127.0.0.1")

        status, headers, response = self.request("HEAD", "/Marti/api/sync/metadata/headhash/tool")

        self.assertEqual(status, 200, response.decode("utf-8", "replace"))
        self.assertEqual(response, b"")
        self.assertEqual(headers["Content-Length"], str(len(payload)))
        self.assertIn('filename="head-map.zip"', headers["Content-Disposition"])
